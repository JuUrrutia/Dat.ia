    ...
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center space-x-2 text-xs w-full sm:w-auto">
          <label htmlFor="catalog-filter-tbl" className="text-slate-600 dark:text-gray-400 font-medium whitespace-nowrap">
            Filtrar por tabla:
          </label>
          <select
            ...
            className="bg-slate-50 dark:bg-dark-base border border-slate-300 dark:border-dark-border rounded-xl px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:border-brand-500 text-xs"
          >
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-dark-border">
        <table className="w-full text-left text-xs min-w-[700px]">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-dark-border text-slate-800 dark:text-gray-200 bg-white dark:bg-transparent">
            {isLoadingCatalog ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-slate-600 dark:text-gray-400">
                  <RefreshCw className="w-5 h-5 animate-spin mx-auto mb-2 text-brand-600 dark:text-purple-400" />
                  Cargando catálogo semántico...
                </td>
              </tr>
            ) : filteredCatalogItems.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-slate-500 dark:text-gray-500">
              ...
              filteredCatalogItems.map((cat) => (
                <tr key={cat.id} className="hover:bg-slate-50 dark:hover:bg-dark-card/50 transition-colors">
                  <td className="px-4 py-3 font-mono font-semibold text-brand-700 dark:text-purple-400">{cat.table_name}</td>
                  <td className="px-4 py-3 font-mono font-medium text-slate-900 dark:text-white">{cat.column_name || '(Toda la tabla)'}</td>
                  <td className="px-4 py-3 text-slate-700 dark:text-gray-300 max-w-md">
                    {cat.friendly_name && (
                      <span className="font-semibold text-slate-900 dark:text-white block mb-0.5">{cat.friendly_name}</span>
                    )}
                    <span>{cat.description}</span>
                  </td>
                  <td className="px-4 py-3 font-mono text-slate-500 dark:text-gray-400 text-[11px]">{cat.business_formula || 'Directa'}</td>
                  <td className="px-4 py-3">
                    {cat.is_ai_generated ? (
                      <span className="bg-brand-50 text-brand-700 border border-brand-200 dark:bg-purple-500/10 dark:text-purple-400 dark:border-purple-500/20 px-2 py-0.5 rounded text-[10px] font-semibold">
                        IA Auto-generado
                      </span>
                    ) : (
                      <span className="bg-slate-100 text-slate-600 border border-slate-200 dark:bg-gray-500/10 dark:text-gray-400 dark:border-gray-500/20 px-2 py-0.5 rounded text-[10px]">
                        Manual Admin
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end space-x-1">
                      <button
                        ...
                        className="p-1.5 rounded-lg text-slate-400 hover:text-brand-600 hover:bg-slate-100 dark:text-gray-400 dark:hover:text-purple-400 dark:hover:bg-purple-500/10 transition-colors"
                        title="Editar Regla"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:text-gray-400 dark:hover:text-rose-400 dark:hover:bg-rose-500/10 transition-colors"
